...
export CROSS_COMPILE="/usr/bin/ccache ./aarch64-linux-elf/aarch64-milouk-elf/bin/aarch64-linux-elf-" ;
export ARCH=arm64;
export SUBARCH=arm;
export KBUILD_BUILD_USER="Akanerin";
export KBUILD_BUILD_HOST="Tamamo-no-Mae";
STRIP="aarch64-linux-elf/aarch64-milouk-elf/bin/aarch64-linux-elf-strip";
export CCOMPILE=$CROSS_COMPILE;
export PATH=$PATH:./aarch64-linux-elf/aarch64-milouk-elf/bin/ ;
make Z00T_defconfig;
make -j$(nproc --all);
mkdir -p tmp_mod;
